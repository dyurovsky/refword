...
```{r test_timecourses, fig.width = 8, fig.height= 6}
...
Plot familiar timecourse for paper
```{r familiar_timecourse, fig.width = 6, fig.height= 3}
#quartz(width = 6, height = 3)
ggplot(filter(test_data_time, Time == round(Time, 1), type == "Familiar"), 
       aes(x = Time, y = roll_mean, colour = age_group, 
           fill = age_group))+
  geom_ribbon(aes(ymin = roll_mean - roll_sem,
                      ymax = roll_mean + roll_sem),
  ...
  theme(panel.grid = element_blank(), legend.position = c(.1,.65),
        axis.title.x = element_text(vjust=-.5), axis.title.y = element_text(vjust=1),
...
# test_mode
# 
#   lm(Novel ~ familiar + age + look_name2 + reach_cotact, data = filter(test_data_subj, group == "child"))
